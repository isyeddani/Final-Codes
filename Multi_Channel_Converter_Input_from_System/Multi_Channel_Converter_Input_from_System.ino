  ...
  if (flag == true)  // flag for interrupt enable
  {
    ...
    if (((int_var > 24000) && (int_var < 25000)) || (int_flag == true))   //if clock cycle is between 24k and 25k then throught and int_flag will help through after
    {
      ...
      if (previous_val != 0 && write_index <= 3000)    // previous cycle will >0 after first interrupt, read data until 3000 counts
      {
        ...
        //        Serial.println(current_val - previous_val);
  ...
  if ((int_flag == true) && (flag == true))   // thorugh if recieve first 24k cycle
  {
    ...
    if (exit_flag > 30000  && exit_flag < 35000)  // True if cycle passed since previous interrupt is greater than 50k
    {
      ...
      if ((flag == false) && (int_flag == false))
      {
        ...
        if ((buffer1[0] > 40750) && (buffer1[0] < 41500) && (logic_holder[0] == 0))
        {
          if ((buffer1[1] > 5750) && (buffer1[1] < 6800) && (logic_holder[1] == 1))
          {
            int i = 2;
            j = 0;
            while (loop_w < 512)
            {
              ...
              //              Serial.println(sum_times1);
              //              Serial.println(buffer1[i]);
              if (sum_times1 >= 256)
              {
                if (((buffer1[i] > 16750) && (buffer1[i + 1] > 750) && (buffer1[i + 1] < 1250) && (buffer1[i + 2] > 22750) && (buffer1[i + 2] < 23250) && (buffer1[i + 3] > 6000) && (buffer1[i + 3] < 7000))  || (sum_times1 >= 256) )
                {
        ...
        previous_val = 0;
        current_val  = 0;
        flag = true;   //flag(false) will skip interrupt
        int_flag = false;
        write_index = 0;
        //                x = x + 1;
        wrong_flag = false;
